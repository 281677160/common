...
if [[ "${REPO_BRANCH}" == *"22.03"* ]]; then
  ...
  rm -fr ${HOME_PATH}/feeds/luci/applications/luci-app-ntpc
   ...
   if [[ "${REPO_BRANCH}" == "2410" ]] && [[ "${SOURCE_CODE}" == "PADAVANONLY" ]]; then
    gitsvn https://github.com/immortalwrt/immortalwrt/blob/openwrt-24.10/package/kernel/linux/modules/usb.mk package/kernel/linux/modules/usb.mk
    ...
    gitsvn https://github.com/immortalwrt/immortalwrt/blob/openwrt-24.10/package/kernel/linux/modules/hwmon.mk package/kernel/linux/modules/hwmon.mk
