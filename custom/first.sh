...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/Namia-R/common build/common
...
git clone -b main --depth 1 https://github.com/Namia-R/OpenWrt-X shangyou
...
curl -fsSL https://raw.githubusercontent.com/Namia-R/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/Namia-R/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="Namia-R/OpenWrt-X"
  sudo rm -rf build common.sh
