...
po_file2="$({ find |grep "/zh-cn/" |grep "\.po" |grep -v "openclash\|store"; } 2>"/dev/null")"
for b in ${po_file2}
do
...
lmo_file="$({ find |grep -E "[a-z0-9]+\.zh_Hans.+lmo" |grep -v "openclash\|store"; } 2>"/dev/null")"
for c in ${lmo_file}
do
...
lmo_file2="$({ find |grep "/zh_Hans/" |grep -v "openclash\|store" |grep "\.lmo"; } 2>"/dev/null")"
for d in ${lmo_file2}
do
...
po_dir="$({ find |grep "/zh-cn" |grep -v "openclash\|store" |sed "/\.po/d" |sed "/\.lmo/d"; } 2>"/dev/null")"
for e in ${po_dir}
do
