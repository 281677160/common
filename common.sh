...
# 定义 TIME 函数（如果它是自定义函数）
function TIME() {
    case "$1" in
        r) echo -e "\033[31m$2\033[0m" ;;  # 红色输出
        g) echo -e "\033[32m$2\033[0m" ;;  # 绿色输出
        *) echo "$2" ;;
    esac
}
...
function Diy_variable() {
    ...
    if [[ -z "${SOURCE_CODE}" ]]; then
        echo "错误：SOURCE_CODE 变量未定义"
        exit 1
    fi
    
    if [[ -z "${HOME_PATH}" ]]; then
        echo "错误：HOME_PATH 变量未定义"
        exit 1
    fi
    
    if [[ -z "${COMPILE_PATH}" ]]; then
        echo "错误：COMPILE_PATH 变量未定义"
        exit 1
    fi
